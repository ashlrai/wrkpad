  ...
  >
    ...
    <ol className="native-truth-grid">
      ...
      <li className={possibleLayerMismatch ? 'problem' : 'pending'}>
        ...
        <small>{possibleLayerMismatch ? 'Cached Ashlr Daily targets shortcuts. Back up, manually Import layer, then drag Codex Native Recovery first (badge 1); Ashlr Daily becomes badge 2. Never Reset.' : 'Select white WIRED mode, then verify first-position badge 1 contains native KV_OAI bindings. A layer number alone is not content proof.'}</small>
      </li>
    ...
    <div className="setup-grid">
      <div className="setup-steps">
        ...
        {nativeShortcutProfileObserved && <div className="native-profile-warning" role="alert" aria-labelledby="native-profile-warning-title">
          <CircleX size={18} />
          <div><strong id="native-profile-warning-title">Possible native-layer mismatch</strong><p>Input’s read-only cache currently names <b>Ashlr Agent Board Corrected · Ashlr Daily</b>. That profile targets Agent Board shortcuts that Codex Native intentionally releases. A black numeric <b>1</b> or layer LED identifies only a layer index; it does not prove native <b>KV_OAI_*</b> bindings. In a trusted Input installation, first export a backup, then use manual <b>Import layer</b> and drag <b>Codex Native Recovery</b> to the first visible position (badge 1); Ashlr Daily becomes badge 2. Wait for <b>Layout updated</b>, export and verify again, then quit Input, select white <b>WIRED</b> mode, and Command-Q and reopen ChatGPT Desktop. A validator finding the native layer anywhere is insufficient: require the human first-position check and this physical acceptance. Never Reset settings or let Agent Board automate the import, reorder, or device write.</p></div>
        </div>}
