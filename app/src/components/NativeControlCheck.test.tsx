...
describe('NativeControlCheck', () => {
  it('explains visible navigation and names the physically correct controls', () => {
    ...
    expect(preflight?.textContent).toMatch(/numeric Layer 1 indicator identifies only a position—not its bindings/i)
    expect(preflight?.textContent).toMatch(/native controls are absent or uncertain.*do not import into or alter the existing profile/i)
    expect(preflight?.textContent).toMatch(/require Setup to show verified Input integrity and preserve rollback exports/i)
    expect(preflight?.textContent).toMatch(/app\/docs\/codex-native-layer-recovery\.md/i)
    expect(preflight?.textContent).toMatch(/new candidate profile.*manual layer import and first-position reorder.*post-import export verification.*human activation/i)
    expect(preflight?.textContent).toMatch(/candidate’s first-position content is verified and you activated it yourself/i)
    expect(preflight?.textContent).toMatch(/Never use Reset settings or let Agent Board import, reorder, activate, or write the device/i)
