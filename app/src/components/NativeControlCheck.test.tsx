...
describe('NativeControlCheck', () => {
  it('explains visible navigation and names the physically correct controls', () => {
    ...
    expect(preflight?.textContent).toMatch(/numeric Layer 1 indicator identifies only a layer position—not its bindings/i)
    expect(preflight?.textContent).toMatch(/manually confirm.*native KV_OAI_\* controls before using short taps/i)
    expect(preflight?.textContent).toMatch(/bindings are absent or unverified.*export a backup.*manual Import layer/i)
    expect(preflight?.textContent).toMatch(/Codex Native Recovery.*first visible position.*badge is 1.*Ashlr Daily becomes badge 2/i)
    expect(preflight?.textContent).toMatch(/Wait for Layout updated.*export and verify again/i)
    expect(preflight?.textContent).toMatch(/native layer found elsewhere in an export is not sufficient/i)
    expect(preflight?.textContent).toMatch(/Never use Reset settings or let Agent Board automate the import, reorder, or device write/i)
