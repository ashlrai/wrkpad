...
test('main status and Flight Check paths await the shared worker-backed inspector', () => {
  ...
  assert.match(source, /const inputInstallation = await inspectCurrentInputInstallation\(forceInput\)[\s\S]*const board = await boardConnected\(\)[\s\S]*const settings = readSettings\(\)[\s\S]*const currentReceiverRuntime = synchronizeShortcutOwnership\(\)/u)
  assert.match(source, /const inputInstallationPending = inspectCurrentInputInstallation\(\)[\s\S]*const currentReceiverRuntime = synchronizeShortcutOwnership\(\)[\s\S]*\[inputInstallation, board,[\s\S]*inputInstallationPending/u)
