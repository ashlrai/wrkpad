...
for(const phrase of ['Developer preview','Synthetic demo','no compliance certification','sets no client-side cookies','not proof of a GitHub Pages deployment'])if(!html.toLowerCase().includes(phrase.toLowerCase()))failures.push(`missing truth phrase ${phrase}`)
for(const phrase of ['Connected is the start','Choose white wired mode','Establish layer 1','within 350 ms','Recover without wiping','Reset settings'])if(!html.toLowerCase().includes(phrase.toLowerCase()))failures.push(`missing first-run phrase ${phrase}`)
for(const utility of ['Karabiner','Logitech Options+'])if(!html.includes(utility))failures.push(`missing official interference check ${utility}`)
if(!html.includes('href="https://worklouder.cc/creator-micro-2"'))failures.push('missing official hardware photography link')
if(!html.includes('href="https://learn.chatgpt.com/docs/features/codex-micro"'))failures.push('missing official Codex Micro setup link')
if(!html.includes('href="https://worklouder.cc/micro-setup"'))failures.push('missing official Work Louder setup link')
for(const phrase of ['Hybrid Native · experimental','twenty action definitions total','fourteen action, stick, and dial shortcuts','six physical Agent keys remain Codex-only','combined 14+6 desk acceptance is still pending'])if(!html.includes(phrase))failures.push(`missing Hybrid Native boundary ${phrase}`)
if(!html.includes('href="https://github.com/ashlrai/wrkpad/blob/main/app/docs/hybrid-native-profile.md"'))failures.push('missing Hybrid Native evidence contract link')
